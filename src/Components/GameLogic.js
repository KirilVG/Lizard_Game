...
class GameLogic {
  constructor(width, height, lanesNum, renderer, gameEndHandler) {
    ...
    this.info = VariableContainer(this.width, this.height, this.lanesNum);
  ...
  animate() {
    if (!this.gameIsOver) {
      ...
      myConstants.setDiscoModeRemainder(
        myConstants.discoModeRemainder - 1 * this.player.valueMultiplier
      );
      ...
      //display score
      this.renderer.displayScoreAndFuel(
  ...
  createConsumableObject() {
    ...
    if (res <= 33) {
      consumable = CollidableObjectFactory.createDiscoConsumable(
        this.info,
        this.lanesNum
      );
    } else {
